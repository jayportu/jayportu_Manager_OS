...
- [x] 🟡 **CAPTCHA Turnstile** en los 4 forms de auth. ✅ código en prod 2026-06-10 (PR #69), **dormido hasta poner llaves**. Login/signup DJ + signup booker + forgot-password pasan `captchaToken` al SDK de Supabase (valida Supabase Auth); `/api/beta` verifica server-side propio. Sin llaves nada cambia (patrón Resend). **Activación = 3 pasos de Jaime (sin deploy):** (1) Cloudflare → Turnstile → Add widget `dropgigs.com` modo Managed; (2) `NEXT_PUBLIC_TURNSTILE_SITE_KEY` + `TURNSTILE_SECRET_KEY` en Vercel (Prod+Preview) → redeploy; (3) Supabase → Auth → Attack Protection → Enable CAPTCHA → Turnstile + el mismo secret. Detalle en `.env.example`.
...
- [ ] 📋 **CSP strict con nonces** (quitar `'unsafe-inline'`/`'unsafe-eval'`) · **2FA** (Supabase TOTP) — pases dedicados.
