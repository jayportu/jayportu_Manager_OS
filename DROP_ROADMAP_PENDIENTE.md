...
- [~] 🟡 **Bookers suspendibles** *(auditoría 2026-06-18)* — **CÓDIGO LISTO (Fase 6, 2026-06-30, rama `feat/fase6-bookers-suspendibles`, build verde), PENDIENTE aplicar migración en prod + merge.** Espeja el patrón de DJs (migración 0030): **migración `0063`** = `account_status` + reason/changed_at/changed_by en `booker_accounts` + trigger `protect_booker_account_status()` (anti auto-reactivación) · `setBookerAccountStatusAction` en admin/actions · `BookerStatusControl` (suspender/banear/reactivar) + badge de estado en `/admin/bookers` · gate en `booker/layout.tsx` (redirige a `/cuenta-suspendida` si suspended/banned) · `/cuenta-suspendida` ahora es role-agnostic (mira dj_profile **y** booker_accounts). **⚠️ Bloqueado en el guardrail:** correr la migración en prod requiere acción de Jaime (`node scripts/run_migration.mjs supabase/migrations/0063_booker_account_status.sql`) ANTES de mergear el PR.
