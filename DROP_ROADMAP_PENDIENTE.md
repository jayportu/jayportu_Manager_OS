...
- [~] ⚪ **QA bajos (72)** — Detalle en `QA_FINDINGS_0611.md`. **~31 arreglados (2026-06-11, PRs #97–#101):** copy/voseo, ~12 `{error}` no chequeados (growth/founding/push/gmail/sync/tracklists/booker/inbox/suscripción/reorder), caps de 1000 (events feed, getTopFollowedDjs, listMyEvents), guards account_status (/api/track + /api/booking), hardening (b/[token] cap+clamp, push rate-limit, resend anti-replay, gmail disconnect same-origin), correctness/SEO (middleware /signup/booker, CRM ?score NaN, JSON-LD, presskit event_date, OG image+canonical /p, counteroffer min-date). **Quedan ~40, todos no-bloqueantes, en 4 grupos:** (a) `alert/confirm` nativos → diálogo propio en 4 vistas de `(app)` (requiere ConfirmProvider en `(app)/layout`); (b) sub-día TZ (`follow-ups nextDueDate`, `datetime-local` con offset del navegador — ±1h, correcto para equipo en Chile); (c) refactors de perf (consumeBetaInvite por-request, listFeedbackReports N llamadas, send-cron N queries, costo del auto-refresh de /trafico); (d) decisiones de producto (assertBetaActive en availability/autopost — podría bloquear a expirados; `pending` da acceso indefinido = decisión de billing; republicar reusa token = por diseño) + cosméticos sueltos (key por índice, next/image guard, sort_order race, comentarios Ollama, versión hardcodeada).
