...
- [ ] 🟡 **M1 · Rate-limit en Cloudflare WAF.** 1 regla gratis: Cloudflare → dropgigs.com → **Security → WAF → Rate limiting rules → Create**. Apunta a los endpoints anónimos (`/api/beta`, `/api/booking`, `/api/event-rsvp`, `/api/track`, `/api/site-track`); ej. **20 req/min por IP → Block/Challenge 10 min**. Protege correos (reputación Resend) + factura Vercel de un flood distribuido, que el rate-limit de-app (en memoria, por instancia) NO frena. *No toca código.* (Ver antecedente: incidente crawler GPTBot, [[vercel_usage_bots_lesson]].) **Reconfirmado por auditoría 2026-07 (hallazgo S-4).** Sugerencia: crear la regla con acción `Log` unos días (Security → Events) para verificar que no atrape tráfico legítimo, y recién ahí subirla a `Managed Challenge`/`Block`.
- [ ] 🔵 **DMARC `p=reject`.** Subir el registro TXT `_dmarc` de `p=none`/`quarantine` → **`p=reject`** una vez que la reputación de envío esté estable (SPF/DKIM ya OK). Endurece contra spoofing del dominio. **Reconfirmado por auditoría 2026-07.** ⚠️ Hacerlo GRADUAL: `dig TXT _dmarc.dropgigs.com` para ver el estado actual; si está en `none`, pasar primero a `quarantine` con `rua=mailto:…` y revisar reportes 2-4 semanas hasta ver 100% de alineación DKIM+SPF; solo entonces `p=reject`. Saltar directo a reject sin datos rebota correos legítimos (invitaciones/notificaciones).
...
- [x] 🔵 **Verificar backups de Supabase.** ✅ **2026-07-05 (auditoría):** plan **Pro**, snapshots físicos diarios funcionando (8 días de retención verificados, 28-jun→5-jul), restaurables. ⚠️ Dos puntos ciegos anotados: (1) los backups **NO incluyen objetos de Storage** (avatares/PDFs de press kit) — si se borra un bucket, la BD no los recupera; evaluar backup/versionado aparte. (2) Backup diario ≠ PITR: evaluar activar **Point-in-Time Recovery** (add-on Pro) para no perder hasta ~24h ante un incidente (relevante con pagos reales).
- [x] 🔵 **Secretos de firma/cifrado en Vercel.** ✅ **2026-07-05 (auditoría):** confirmadas en Production `UNSUBSCRIBE_SECRET` (cierra S-2 — los links de baja ya no se firman con el service-role key) y `GMAIL_TOKEN_ENC_KEY` (cierra S-7 — tokens de Gmail cifrados en reposo; backfill ya aplicado en batch B privacy).
...
- [ ] ⚪ Paginación CRM a escala · acciones masivas · ~~duplicar plantilla~~ ✅ (hecho 2026-07-01: botón "Duplicar" en `/plantillas` → clona con "(copia)" y lleva a editarla; verificado end-to-end) · ~~botón PNG tracklist (BUG-02)~~ (resuelto: el botón se escondió) · ~~drill-down posts→campaña en Growth~~ (ya está: el board de posts muestra la campaña de cada post) · **bonus: duplicar post** ✅ (hecho 2026-07-02: botón "Duplicar" en el board de Growth → clona el contenido como borrador nuevo; verificado end-to-end). *(Ya hechos en sesiones recientes: validación email/WhatsApp server-side en contactos —2026-06-11—; dedup/idempotencia en crons —growth sync + MP webhook—.)*
...
- [x] 🆕 **"Elige tu perfil" en la landing + higiene del press kit (2026-07-02, PRs #199–#202).** Aterriza el "selector de rol próximamente" que estaba **diferido** (ver Rebrand dark ↓): DROP comunica su visión de plataforma sin ofrecer signups que no existen aún.
  - **Sección `#perfiles`** en el landing (`role-picker.tsx`, entre "Suena ahora" y el FAQ): 4 tarjetas — **DJ** activo (→ `/beta`) + **Booker / Fotógrafo / Audiovisual** como "próximamente" (no clickeables). Los roles futuros = los que ya promete el FAQ, no inventados (#200).
  - **CTA del header = desplegable "Elige tu perfil"** (`header-profile-menu.tsx`, client component): DJ → `/beta`; el resto baja a `#perfiles`. Label responsive (desktop "Elige tu perfil" / móvil "Perfil"). Cierra al elegir, click-afuera o Escape (#201 lo introdujo como `<details>`; #202 lo migró a client porque el `<details>` se quedaba abierto en anclas de la misma página). Footer: "Soy booker" ya no manda al signup vivo → `/#perfiles` (ruta `/signup/booker` sigue viva para invites directos).
  - **Press kit — ocultar links basura (#199):** helper `isRenderableLink()` en `lib/format` → un campo social que no sea URL válida (vacío, o nombre con espacios tipo "soundcloud.com/Pablo Rocha" que da 404) se trata como "no puesto" y **no se renderiza** (ni embed, ni botón "Ver en SoundCloud →", ni `sameAs` del JSON-LD). Aplica a soundcloud/youtube/spotify/beatport/bandcamp/website/instagram + sets destacados. *(Relacionado con el pendiente MANUAL de FASE 0: mientras Pablo Rocha / Milo Varas / NICO VILLEGAS no den su handle real, su SoundCloud simplemente no aparece — limpio, en vez de link roto.)*
  - **Modelo de login (documentado):** `/login` es **un solo form genérico** (email+contraseña o Google, sin elegir rol). El ruteo post-auth discrimina por **tipo de cuenta del usuario** (no por el texto del email): `RootPage`/`/auth/callback` → `dj_profile`→`/dashboard`, `booker_accounts` o `account_type=booker`→`/booker/requests`, ninguno→`/welcome`. **Para activar un rol futuro** (fotógrafo/audiovisual) basta sumar una rama de ruteo (ej. `photographer_profile → /studio`) + su tabla/onboarding; la base ya está lista.
- [ ] 🎨 **Perfiles creativos (fotógrafo / audiovisual) — DISEÑADOS 2026-07-02 · construir para lanzamiento.** Mockup aprobado por Jaime (`drop_perfiles_creativos_mockup.html` en la raíz, 4 tabs · marca dark; también publicado como Artifact). **Arquitectura decidida:**
  - **1 perfil "creativo visual"** con `services[]` (`foto` y/o `video`, extensible a streaming/diseño). Una sola cuenta; al onboarding marca qué ofrece; aparece en el/los directorio(s) que corresponda (`/fotografos`, `/audiovisual` = vistas filtradas por service). Resuelve el cruce foto+video sin duplicar (decisión de Jaime vs. "dos perfiles + casilla").
  - **Perfil público** = molde del press kit del DJ: portafolio (álbumes foto / reels video), **paquetes con precio + entregables/tiempos**, disponibilidad (reusa calendario DJ), equipo, radio de viaje, verificado, contacto **gated** a DJs/bookers.
  - **Flujo DJ→creativo "solicitar fecha"** = clona el motor de booking existente (booking_requests / el patrón `venue_pitches`): el DJ pide fecha+paquete, el creativo **acepta / propone cambios / rechaza**; al aceptar entra a ambos calendarios y libera contacto (DROP NO intermedia el pago, igual que con DJs).
  - **Ideas de experto priorizadas:** (1) **créditos cruzados con la red de DJs** — el creativo etiqueta al DJ, aparece en ambos perfiles, se auto-distribuye (moat, nadie más tiene la red); (2) paquetes con precio+tiempos; (3) reseñas post-gig (v2); (4) radio de viaje.
  - **Pendiente de construir:** tabla `creative_profile`, migración, directorios filtrados, tabla de solicitudes, onboarding de servicios, activar los botones del landing (hoy "próximamente"). Falta escribir el **spec técnico** → plan de implementación cuando se decida abrir.
...
>
> **▶ Estado al 2026-07-02 (cierre de sesión):** fase **TRACCIÓN**. Producto (Fases 0–5), **hardening (Fase 6 COMPLETA)** y **auditoría de portada** cerrados y en prod. Cerrado en esta tanda (PRs #188–#197): Fase 6 — anti DNS-rebinding, revoke + **cifrado de tokens Gmail** (dormido tras `GMAIL_TOKEN_ENC_KEY`, ya ON), **bookers suspendibles** (migr 0063); portada — **#3 demo visual "Míralo por dentro"** (capturas reales de cuenta demo), **#5 copy beneficios**, **#6 tooltip "verificado"**; **logout → landing**; **pulso + digest** activos (`PULSO_DIGEST_TO` ON); quick-wins **duplicar plantilla** + **duplicar post**. **Landing (PRs #199–#202):** sección **"Elige tu perfil"** (`#perfiles`: DJ activo + Booker/Fotógrafo/Audiovisual "próximamente") + **CTA del header como desplegable** de perfiles; **higiene del press kit** (`isRenderableLink` oculta links basura/vacíos, ej. SoundCloud con nombre en vez de handle). Flags ON confirmadas: `BETA_AUTO_APPROVE_ENABLED`, `ONBOARDING_NUDGE_ENABLED`, `PULSO_DIGEST_TO`, `GMAIL_TOKEN_ENC_KEY`. **NO queda deuda que bloquee ni quick-wins de código de valor.** Lo pendiente es: escala futura (Smart Match v2, RA-4, paginación/acciones masivas CRM), **pausado por decisión** (Fase 4 cobros), **checklist de lanzamiento** (M1 Cloudflare, DMARC, rotar Resend, backups, Legal #7 — paneles externos), y **manual tuyo**. **▶ SIGUIENTE GRAN PASO = TRACCIÓN: activar los ~13 DJs (perfil completo + 1 evento).** El producto está sobrado para la escala actual; el cuello es distribución, no código. *(Foto visual de pendientes: artifact "Roadmap · lo que falta", 1-jul.)*
>
> **▶ Estado al 2026-07-06 (actualización con trabajo posterior al 2-jul):** el proyecto se movió a `~/Desktop/DROP/jayportu_Manager_OS/` y `main` avanzó a **#220**. Además de lo del 2-jul, se sumó (PRs #206–#220): **Auditoría integral 2026-07** (#216/#218 — hardening, observabilidad, UX pública, suite de tests; WAF/DMARC + backups Supabase + secretos verificados, detalle en el checklist ↑) · **verificación automática de DJs** (#211–#214, migración 0066: trigger event-driven, dual-modo n8n/directo a `/evaluate`, umbral de bio a 40) · **vista "Cobros" en `/calendario`** (#206/#210) = **visibilidad de ingresos del DJ** (KPIs por-cobrar / cobrado / **proyectado** por mes, aging, "✓ marcar pagado"; cuenta cualquier evento con monto, no solo `show`) — es **tracking que ve el DJ, NO procesa pagos**: ⏸️ **Fase 4 (cobros reales / suscripción vía MercadoPago) SIGUE EN PAUSA** (confirmado por Jaime, 6-jul) · **fixes de auth del caso real de un DJ (Felipe)** — campo "confirmar contraseña" en signup (#207, atrapa typos que dejaban al DJ sin poder entrar) + botón de recuperación de clave visible al fallar el login (#209) · **refactor de emails** (#219, hallazgo T-3: `templates.ts` de 2597 líneas → módulos temáticos vía barrel, output byte-idéntico, cero cambio de comportamiento) · **marca** (#220): `#E85A0C` fijado como único naranjo canónico (solo docs, prod ya lo usaba) y **"Soy booker" eliminado del footer** (el rol sigue "próximamente" y su signup es solo por invitación founding; supera el `/#perfiles` que había puesto #200). **▶ SIGUIENTE GRAN PASO no cambia: TRACCIÓN — activar los DJs.**
>
