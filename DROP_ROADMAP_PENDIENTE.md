...
- [ ] 🟡 **Encriptar los tokens de Gmail en reposo** *(auditoría de seguridad 2026-06-13)* — `gmail_connections` guarda los tokens OAuth de Google (acceso al correo del DJ) en **texto plano**. Tras PR #129 ya **no** son leíbles desde el cliente (solo `service_role` en el servidor), pero si se filtrara un backup de la DB o la `SUPABASE_SERVICE_ROLE_KEY`, serían usables directo. Encriptar con **AES-256-GCM** (nuevo secreto `GMAIL_TOKEN_ENC_KEY` + backfill de las filas existentes) o usar **Supabase Vault / pgsodium**. Defensa en profundidad — no urgente.
- [ ] 🟡 **Revocar el token en Google al desconectar Gmail** *(auditoría de seguridad 2026-06-13)* — hoy `/api/gmail/disconnect` solo borra la fila de `gmail_connections`; el token sigue **válido en Google** hasta que el user lo revoque a mano. Llamar a `https://oauth2.googleapis.com/revoke` en el disconnect. Conviene hacerlo junto con el punto anterior. *(Contexto: la auditoría 2026-06-13 cerró lo crítico/alto/medio — escalada a admin, webhook MP, SSRF, tokens Gmail solo-servidor, unsubscribe firmado — en PRs #126–#129. Estos dos son los pendientes de baja prioridad que quedaron.)*
...
- [ ] 🟡 **M1 · Rate-limit público respaldado por infra** *(auditoría 2026-06-18)* — el rate-limit actual (agregado 2026-06-08) vive **en memoria por instancia** → se resetea en cold start y se duplica entre instancias autoescaladas, así que no frena un ataque distribuido contra los endpoints anónimos (`/api/beta`, `/api/booking`, `/api/event-rsvp`, `/api/track`, `/api/site-track`). Respaldar con **regla de rate-limit en Cloudflare WAF** (ya hay Cloudflare delante; el plan free incluye 1 regla) o **Upstash Redis**. *(Requiere acción de Jaime en el panel de Cloudflare — más alto-impacto de lo que queda.)*
- [ ] 🟡 **M2 · Anti DNS-rebinding en `url-guard`** *(auditoría 2026-06-18)* — `isSafePublicHttpUrl()` bloquea localhost/IPs privadas/metadata por **hostname** pero NO resuelve DNS → un dominio público que resuelve a una IP interna podría pasar (SSRF). Afecta `tracklist/notify` y `webhook/test`, **ambos autenticados** (riesgo acotado). Fix: resolver el host y validar la IP resuelta antes de conectar, o salida por proxy/allowlist.
...
- [ ] 🟡 **Bookers suspendibles** *(descubierto en auditoría 2026-06-18 — Jaime confirmó que SÍ deberían poder suspenderse)* — hoy los bookers **no se pueden suspender/banear**: `booker_accounts` no tiene `account_status` y `assertBetaActive` solo lee de `dj_profile` (que el booker no tiene). Hacerlos moderables como los DJs = migración (columna `account_status` + guard anti-bypass tipo `protect_account_status` + INSERT-guard) + acción admin (extender `setAccountStatusAction`) + chequeo en `booker/layout.tsx` (espejo del de `(app)/layout.tsx`). Es **feature**, no quick win.
...
- [x] 🤖 **ACTIVAR — secuencia de correos de ciclo de vida (PR #179/#180, en prod, migración 0062).** Los 3 correos que mueven el North Star (DJ con press kit vivo): **E1 bienvenida** (transaccional, al completar el onboarding; one-shot `welcome_email_sent_at`) · **E2 nudge** (el cron que ya existía dormido, copy reescrito al framing "press kit"; sigue gateado por `ONBOARDING_NUDGE_ENABLED`) · **E3 press kit vivo** (transaccional, la 1ª vez que el perfil queda live-ready vía `isPresskitLiveReady`; one-shot `presskit_live_email_sent_at`). Best-effort: si Resend falla no rompe el onboarding/guardado. Copy 100% tuteo chileno; mockups aprobados por Jaime. **Pendiente manual:** (a) QA de entrega real con una cuenta de prueba (no se pudo probar sin sesión); (b) encender E2 con `ONBOARDING_NUDGE_ENABLED=true`.
- [x] 🎨 **Rebrand dark + producto (jun 25–30, en prod).** Logo nuevo DROP. en Satoshi (#164/#165) + portada/landing dark (#166–#168) + dashboard detrás de flag (#167) · calendario **vista MES** + toggle Lista/Mes (#170) · **galería de fotos** del press kit con carpetas + lightbox, migración 0061 (#172) · aviso al admin solo cuando el correo de aprobación **no** sale (#173) · copy landing quick-wins (#177) · fix de ruido de Sentry del navegador in-app de IG/iOS (#178).
...
- [ ] 🆕 **#3 · Mostrar el producto en la portada (demo visual).** Hoy la portada explica pero no demuestra; los `FeatureTabs` son ilustrativos. Falta una sección con **capturas/demo reales** de Dashboard, CRM, Calendario, Press kit, Inbox de bookings, Perfil público y Métricas — idealmente un loop/demo de 30-45s. *(Trabajo grande: requiere generar los assets. Sesión dedicada.)*
- [ ] 🆕 **#5 · Traducir funciones → beneficios económicos.** El copy describe features, no resultados. Reescribir hacia el beneficio: CRM = no perder oportunidades · Agenda = saber cuánto ganas al mes · Press kit = presentarte profesional en segundos · Booking inbox = cotizar y dar seguimiento sin desorden · Métricas = saber si tu carrera crece. (Cambio de copy en `src/components/public/landing/*` + FAQ.)
- [ ] 🆕 **#6 · Criterio visible de "DJ verificado".** El badge "✓ Verificado por DROP." sale en `/p/[slug]:297` y `/dj:415` sin explicar qué significa. Agregar leyenda/tooltip público, ej. "Identidad e Instagram revisados por DROP." (El bloque granular Identidad/Redes/Sets ya existe en el perfil, `/p/[slug]:141-143`, pero el badge principal y el filtro de `/dj` no lo explican.) Cambio chico de copy + tooltip.
...
>
> **▶ Estado al 2026-06-30:** fase **TRACCIÓN**. La fuga #1 (gate de la beta) quedó cerrada con **auto-aprobación same-day (#176)**, y la capa de activación ya está en prod: **funnel de conversión (#175)** + **secuencia de correos E1/E2/E3 (#179/#180)** + pulso (#124) + landing UX (#122). **Manual de Jaime ahora:** (1) **QA de entrega real** de los correos de activación con una cuenta de prueba; (2) encender los flags dormidos en Vercel — `BETA_AUTO_APPROVE_ENABLED=true` (auto-aprobación), `ONBOARDING_NUDGE_ENABLED=true` (E2/nudge) y `PULSO_DIGEST_TO` (digest del pulso); (3) empujar a los ~13 DJs a perfil completo + 1 evento. En pausa: Founding Bookers. Siguiente candidato de producto: **#3 demo visual de la portada** (ver auditoría ↑).
>
