...
function GroupDivider({ children }: { children: React.ReactNode }) {
  return (
    <div className="inline-block bg-ink text-cream font-mono text-[11px] font-bold uppercase tracking-[0.1em] px-3 py-1.5 mt-10 mb-4">
      {children}
    </div>
  );
}
...
const COVERS: { t: string; svg: React.ReactNode }[] = [
  { t: "Subsuelo", svg: (<><rect width="100" height="100" fill="#0A0A0A" /><circle cx="50" cy="40" r="27" fill="#FF5C00" /><rect y="63" width="100" height="3" fill="#F4EFE7" /><rect y="71" width="100" height="1.5" fill="#F4EFE7" opacity=".5" /><rect y="77" width="100" height="1" fill="#F4EFE7" opacity=".3" /></>) },
  { t: "Cobre", svg: (<><rect width="100" height="100" fill="#FF5C00" /><polygon points="0,100 100,0 100,100" fill="#0A0A0A" /><line x1="0" y1="100" x2="100" y2="0" stroke="#F4EFE7" strokeWidth="2" /></>) },
  { t: "Madrugada", svg: (<><rect width="100" height="100" fill="#F4EFE7" /><circle cx="50" cy="50" r="34" fill="none" stroke="#0A0A0A" strokeWidth="2" /><circle cx="50" cy="50" r="22" fill="none" stroke="#0A0A0A" strokeWidth="1" /><circle cx="50" cy="50" r="6" fill="#FF5C00" /></>) },
  { t: "Ritual", svg: (<><rect width="100" height="100" fill="#F4EFE7" /><polygon points="50,16 84,84 16,84" fill="#0A0A0A" /><polygon points="50,44 67,84 33,84" fill="#FF5C00" /></>) },
  { t: "Niebla", svg: (<><rect width="100" height="100" fill="#2b2f36" /><rect y="30" width="100" height="2" fill="#F4EFE7" opacity=".85" /><rect y="40" width="100" height="2" fill="#F4EFE7" opacity=".6" /><rect y="50" width="100" height="3" fill="#FF5C00" /><rect y="61" width="100" height="2" fill="#F4EFE7" opacity=".4" /><rect y="70" width="100" height="2" fill="#F4EFE7" opacity=".25" /></>) },
  { t: "Tránsito", svg: (<><rect width="100" height="100" fill="#0A0A0A" /><rect x="20" y="20" width="60" height="60" fill="none" stroke="#FF5C00" strokeWidth="3" /><rect x="34" y="34" width="32" height="32" fill="none" stroke="#F4EFE7" strokeWidth="2" /><rect x="46" y="46" width="8" height="8" fill="#FF5C00" /></>) },
];
    ...
    <section className="border-y-2 border-ink bg-cream">
      <div className="max-w-[1140px] mx-auto px-6 py-16">
        ...
        <p className="text-[16px] text-fg-muted" style={{ maxWidth: "60ch" }}>
          Un link vivo que te consigue bookings <strong className="text-ink">y</strong> te ayuda a gestionarlos —
        ...
        <div ref={vitrina.ref} className="grid md:grid-cols-2 gap-4">
          ...
          <Reveal show={vitrina.inView} delay={0} className="h-full">
            <div className={CARD}>
              ...
              <div className="mt-auto border border-ink overflow-hidden">
          ...
          <Reveal show={vitrina.inView} delay={90} className="h-full">
            <div className={CARD}>
              ...
              <div className="mt-auto border border-ink bg-cream p-3">
                ...
                <div className="grid grid-cols-7 gap-1">
                  {calDays.map((s, i) => (
                    <div
                      key={i}
                      className={`aspect-square flex items-center justify-center font-mono text-[11px] tabular-nums transition-transform duration-300 ${s === 1 ? "bg-orange text-ink font-bold" : s === 2 ? "bg-ink text-cream/70 line-through" : "text-fg-subtle"} ${vitrina.inView ? "scale-100" : "scale-0"}`}
                ...
                <div className="flex gap-4 mt-2.5 font-mono text-[9px] uppercase tracking-wider text-fg-muted">
                  <span className="inline-flex items-center gap-1.5"><span className="w-2.5 h-2.5 bg-orange border border-ink" />Disponible</span>
          ...
          <Reveal show={vitrina.inView} delay={270} className="h-full">
            <div className={CARD}>
              ...
              <div className="mt-auto border border-ink bg-cream p-4 flex flex-col gap-3">
                {MATCH.map((m) => (
        ...
        <p className="text-[15px] text-fg-muted mb-4" style={{ maxWidth: "64ch" }}>
          Una vitrina te consigue <strong className="text-ink">un</strong> gig. DROP te ayuda a construir una{" "}
          <strong className="text-ink">carrera</strong>: ordena a tus contactos, tus fechas y tu plata — lo que haría un manager, pero es tuyo.
        </p>
        <div ref={back.ref} className="grid md:grid-cols-2 gap-4">
          ...
          <Reveal show={back.inView} delay={0} className="h-full">
            <div className={CARD}>
              ...
              <div className="mt-auto flex flex-col gap-2">
                {CRM.map((r) => (
                  <div key={r.name} className="flex items-center gap-2 flex-wrap border border-ink bg-cream px-2.5 py-2">
                    <span className="font-mono text-[8px] font-bold uppercase tracking-wider border border-ink px-1.5 py-0.5">{r.tag}</span>
          ...
          <Reveal show={back.inView} delay={90} className="h-full">
            <div className={CARD}>
              ...
              <div className="mt-auto border border-ink bg-cream p-4 flex items-end gap-6 flex-wrap">
                <div>
          ...
          <Reveal show={back.inView} delay={180} className="md:col-span-2 h-full">
            <div className={CARD}>
              ...
              <div className="mt-auto border border-ink bg-cream flex flex-wrap">
                <div className="flex-1 min-w-[170px] border-r border-ink p-3.5">
                  ...
                  <span className="inline-block mt-2 font-mono text-[9px] font-bold uppercase border border-ink bg-orange text-ink px-1.5 py-0.5 animate-pulse">● Pendiente</span>
                </div>
                <div className="flex-1 min-w-[170px] border-r border-ink p-3.5">
                  ...
                  <span className="inline-block mt-2 font-mono text-[9px] font-bold uppercase border border-ink bg-cream text-ink px-1.5 py-0.5">✓ Cerrado</span>
                </div>
        ...
        <div className="mt-12 flex flex-wrap items-center gap-4">
          <Link href="/beta" className="inline-flex items-center gap-2.5 px-6 py-3.5 bg-orange text-ink border-2 border-ink font-mono text-[12px] font-bold uppercase tracking-[0.14em] hover:bg-ink hover:text-orange transition-colors">
